...
if (!is_admin()) {
    if (has_permission('lead_manager', '', 'view_own')) {
        array_push($where, 'AND (assigned =' . get_staff_user_id() . ' OR addedfrom = ' . get_staff_user_id().') ');
    }
}
...
foreach ($rResult as $aRow) {
    ...
    $allow_whatsapp = '<li><a href="https://web.whatsapp.com/send?phone=+'.$aRow['phonenumber'].'" target="_blank" ><i class="fa fa-whatsapp" aria-hidden="true"></i></a></li>';
    ...
    $row[] = '<ul class="list-inline"><li>' . $allow_call . $allow_video_call . $allow_sms . $allow_email . $no_permission .$allow_whatsapp. '</li></ul>';
    if (is_gdpr() && $consentLeads == '1') {
