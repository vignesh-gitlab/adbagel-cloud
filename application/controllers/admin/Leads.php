...
class Leads extends AdminController
{
    ...
    public function campaigntable()
    {
        ...
        $this->app->get_table_data('campaign');
    }
    ...
    public function lead($id = '')
    {
        ...
        // if (!staff_can('edit', 'leads') && !staff_can('create', 'leads')) {
        //     ajax_access_denied();
        // }
    ...
    public function campaign($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($this->input->post('id')=='') {
                ...
                $id      = $this->leads_model->addCampaign($this->input->post());
                $message = $id ? _l('added_successfully', 'Campaign') : '';
                ...
                unset($data['id']);
                $message         = '';
                $success         = $this->leads_model->updateCampaign($data, $id);
                
                
                if ($success) {
                    
                    $message = _l('updated_successfully', 'Campaign');
                }
                echo json_encode([
                    'success'          => $success,
                    'message'          => $message,
    ...
    public function managecampaign($id = '')
    {
        ...
        $data['campaignid'] = $id;
        $this->load->view('admin/leads/manage_campaign', $data);
    }
    ...
    public function campaignDelete($id)
    {
        ...
        // }
        if (!$id) {
            redirect(admin_url('leads/managecampaign'));
        }
        $this->load->model('expenses_model');
        $response = $this->expenses_model->deleteCampaign($id);
        
        if ($response === true) {
            set_alert('success', _l('deleted', 'Campaign'));
        } else {
            
                set_alert('warning', _l('problem_deleting', 'campaign'));
    ...
    public function campaignbulk_action()
    {
        if (!is_staff_member()) {
            ajax_access_denied();
        }

        hooks()->do_action('before_do_bulk_action_for_campaign');
        $total_deleted = 0;
        if ($this->input->post()) {
            $ids                   = $this->input->post('ids');
            
            $has_permission_delete = has_permission('campaign', '', 'delete');
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if ($has_permission_delete) {
                            if ($this->leads_model->campaigndelete($id)) {
                                $total_deleted++;
                            }
                        }
                    } 
        ...
        if ($this->input->post('mass_delete')) {
            set_alert('success', _l('Campaign deleted', $total_deleted));
        }
    }
    ...
    public function campaignimport()
    {
        if (!has_permission('campaign', '', 'create')) {
            access_denied('campaign');
        }

        

        $dbFields = $this->db->list_fields(db_prefix() . 'campaign_manager');

        $this->load->library('import/import_campaign', [], 'import');
        ...
        $this->load->view('admin/leads/campaignimport', $data);
    }
