...
class Projects_model extends App_Model
{
    ...
    public function share($id, $data)
    {
        ...
        $clientid_copy_project = $data['clientid_copy_project'];
        
        if ($send_created_email == true) {
            $this->send_matter_customer_email($id,$clientid_copy_project, 'project_created_to_customer');
        }
    }
    
    public function send_matter_customer_email($id, $clientid, $template)
    {
        $this->db->select('contact_notification,notify_contacts');
        $this->db->where('id', $id);
        $project = $this->db->get(db_prefix() . 'projects')->row();
        ...
        if ($project->contact_notification == 1) {
            $contacts = $this->clients_model->get($clientid, ['active' => 1]);
        } else {
            $contacts = [];
        }
