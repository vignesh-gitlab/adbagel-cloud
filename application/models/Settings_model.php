    ...
    public function update($data)
    {
        ...
        if(!is_admin())
        {
            if(isset($data['appId']))
            {
                ...
                $dataa['appId'] = $data['appId'];
                $dataa['appSecret'] = $data['appSecret'];
                $dataa['facebookPageId'] = $data['facebookPageId'];
                $dataa['appAccessToken'] = $data['appAccessToken'];
                $dataa['addedfrom'] = get_staff_user_id();
                $this->db->where('addedfrom',get_staff_user_id());
                $facebook = $this->db->get(db_prefix() . 'facebook_integeration')->row();
                if($facebook)
                {
                    $this->db->where('id',$facebook->id);
                    $this->db->update(db_prefix().'facebook_integeration',$dataa);
                    $affectedRows = $this->db->affected_rows();
                    return (bool) $affectedRows;
                }else
                {
                    $this->db->insert(db_prefix().'facebook_integeration',$dataa);
