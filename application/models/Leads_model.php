...
class Leads_model extends App_Model
{
    ...
    public function addCampaign($data)
    {
        ...
        $this->db->insert(db_prefix() . 'campaign_manager', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            
            log_activity('New Campaign Added [' . $data['campaign'] . ', ID: ' . $insert_id . ']');
        }
    ...
    public function updateCampaign($data,$id)
    {
        ...
        $this->db->where('id',$id);
        $this->db->update(db_prefix() . 'campaign_manager', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Campaign Updated [Name: ' . $data['campaign'] . ', ID: ' . $id . ']');
    ...
    public function campaigndelete($id)
    {
        ...
        hooks()->do_action('before_campaign_deleted', $id);

        

        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'campaign_manager');
        if ($this->db->affected_rows() > 0) {
            log_activity('Campaign Deleted [Deleted by: ' . get_staff_full_name() . ', ID: ' . $id . ']');
